...
final class Bounties extends Module {
	...
	public function title(): string {
		return __( 'Bounties', 'wp-mafia-game' );
	}
	...
	public function settings_fields(): array {
		return array(
			'bounty_min'            => array(
				'label'   => __( 'Minimum bounty', 'wp-mafia-game' ),
				'type'    => 'int',
				'default' => 10000,
			),
			'bounty_buyoff_percent' => array(
				'label'       => __( 'Buy-off cost (% of the bounties)', 'wp-mafia-game' ),
				'type'        => 'int',
				'default'     => 150,
			),
			'bounty_cancel_refund'  => array(
				'label'   => __( 'Refund when withdrawing (%)', 'wp-mafia-game' ),
				'type'    => 'int',
				'default' => 50,
			),
		);
	...
	public function pay_out( Character $victim, ?Character $killer ): void {
		...
		if ( $killer && $total > 0 ) {
			...
			$killer->notify( sprintf( __( 'You received a bounty of %1$s for the murder of %2$s.', 'wp-mafia-game' ), esc_html( Format::money( $total ) ), esc_html( $victim->name ) ) );
		}
	}
	...
	public function menu( Character $c ): array {
		...
		return array(
			array(
				'label' => __( 'Bounties', 'wp-mafia-game' ),
				'group' => 'murder',
				'order' => 30,
				'badge' => $on_me ? '!' : '',
			),
	...
	public function action_place( Character $c, array $input ): void {
		...
		if ( $target->id() === $c->id() ) {
			$this->error( __( 'A bounty on yourself? Better not.', 'wp-mafia-game' ) );
			return;
		}
		if ( $amount < $min ) {
			/* translators: %s: money */
			$this->error( sprintf( __( 'The minimum bounty is %s.', 'wp-mafia-game' ), Format::money( $min ) ) );
			return;
		}
		...
		$by = $anonymous ? esc_html__( 'Someone', 'wp-mafia-game' ) : $c->link();
		/* translators: 1: player, 2: money */
		$target->notify( sprintf( __( '%1$s put a bounty of %2$s on your head!', 'wp-mafia-game' ), $by, esc_html( Format::money( $amount ) ) ) );
		$c->log( 'bounty.place', true, $amount, $target->id() );
		/* translators: 1: money, 2: player */
		$this->success( sprintf( __( 'There is now %1$s on the head of %2$s.', 'wp-mafia-game' ), Format::money( $amount ), $target->name ) );
	}
	...
	public function action_buyoff( Character $c, array $input ): void {
		...
		if ( ! $total ) {
			$this->error( __( 'There is no bounty on your head.', 'wp-mafia-game' ) );
			return;
		}
		$cost = (int) ceil( $total * (int) $this->setting( 'bounty_buyoff_percent' ) / 100 );
		if ( ! $c->spend( 'money', $cost ) ) {
			/* translators: %s: money */
			$this->error( sprintf( __( 'Buying yourself free costs %s in cash.', 'wp-mafia-game' ), Format::money( $cost ) ) );
			return;
		}
		DB::delete( 'bounties', array( 'target_id' => $c->id() ) );
		$c->log( 'bounty.buyoff', true, $cost );
		$this->success( __( 'You bought off all bounties on your head.', 'wp-mafia-game' ) );
	}
	...
	public function action_cancel( Character $c, array $input ): void {
		...
		if ( DB::delete( 'bounties', array( 'id' => $row['id'] ) ) ) {
			...
			$c->add( 'money', $refund );
			/* translators: %s: money */
			$this->success( sprintf( __( 'Bounty withdrawn. You got %s back.', 'wp-mafia-game' ), Format::money( $refund ) ) );
		}
	}
}
