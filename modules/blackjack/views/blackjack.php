 ...
 * @var array|null                     $game
 * @var \DigiFalk\UnderworldEmpire\Property  $table
 * @var int                            $min
 * @var int                            $max
 *
...
use DigiFalk\UnderworldEmpire\Modules\Blackjack;
...
<div class="dfmg-card dfmg-blackjack">
	<?php if ( $game ) : ?>
		<p><?php esc_html_e( 'Bet:', 'underworld-empire' ); ?> <strong><?php echo esc_html( Format::money( $game['bet'] ) ); ?></strong></p>
		<h4><?php esc_html_e( 'Bank', 'underworld-empire' ); ?></h4>
		<?php echo $dfmg_cards( $game['dealer'], true ); // phpcs:ignore ?>
		<h4><?php esc_html_e( 'You', 'underworld-empire' ); ?> (<?php echo (int) Blackjack::score( $game['player'] ); ?>)</h4>
		<?php echo $dfmg_cards( $game['player'] ); // phpcs:ignore ?>
		<div class="dfmg-actions">
			<?php echo $this->button( 'hit', __( 'Hit', 'underworld-empire' ) ); // phpcs:ignore ?>
			<?php echo $this->button( 'stand', __( 'Stand', 'underworld-empire' ), array(), 'dfmg-button dfmg-button--ghost' ); // phpcs:ignore ?>
		</div>
	<?php else : ?>
		<?php if ( $dfmg_last ) : ?>
			<div class="dfmg-bj-last">
				<h4><?php esc_html_e( 'Bank', 'underworld-empire' ); ?> (<?php echo (int) Blackjack::score( $dfmg_last['dealer'] ); ?>)</h4>
				<?php echo $dfmg_cards( $dfmg_last['dealer'] ); // phpcs:ignore ?>
				<h4><?php esc_html_e( 'You', 'underworld-empire' ); ?> (<?php echo (int) Blackjack::score( $dfmg_last['player'] ); ?>)</h4>
		...
		<?php echo $this->form( 'bet' ); // phpcs:ignore ?>
			<label>
				...
				printf( esc_html__( 'Bet (%1$s – %2$s)', 'underworld-empire' ), esc_html( Format::money( $min ) ), esc_html( Format::money( $max ) ) );
				?>
				<input type="text" inputmode="numeric" name="bet" value="<?php echo esc_attr( (string) ( $dfmg_last['bet'] ?? $min ) ); ?>">
			</label>
			<button type="submit" class="dfmg-button"><?php esc_html_e( 'Deal', 'underworld-empire' ); ?></button>
		</form>
	<?php endif; ?>
</div>
