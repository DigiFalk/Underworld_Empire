 ...
 * Description: Richt een familie op of sluit je aan. Met rangen (baas, onderbaas), rechten per lid, een gezamenlijke kas voor geld en kogels, uitnodigingen, logboek en uitbreidbare ledenlimiet.
 * Version: 1.0.0
 * Author: DigiFalk
 *
...
final class Families extends Module {
	...
	public function settings_fields(): array {
		return array(
			'family_cost'          => array(
				'label'   => __( 'Kosten om een familie op te richten', 'wp-maffia-game' ),
				'type'    => 'int',
				'default' => 1000000,
			),
			'family_one_per_city'  => array(
				'label'   => __( 'Maximaal één familie per stad', 'wp-maffia-game' ),
				'type'    => 'checkbox',
				'default' => 1,
			),
			'family_base_capacity' => array(
				'label'   => __( 'Basis aantal leden', 'wp-maffia-game' ),
				'type'    => 'int',
				'default' => 5,
			),
			'family_upgrade_cost'  => array(
				'label'       => __( 'Uitbreidingskosten per niveau', 'wp-maffia-game' ),
				'type'        => 'int',
				'default'     => 250000,
				'description' => __( 'Kosten = niveau × dit bedrag, betaald uit de familiekas. Elk niveau geeft 1 extra plek.', 'wp-maffia-game' ),
			),
			'family_cash_tax'      => array(
				'label'   => __( 'Witwaskosten storten geld (%)', 'wp-maffia-game' ),
				'type'    => 'int',
				'default' => 15,
			),
			'family_bullet_tax'    => array(
				'label'   => __( 'Verlies bij storten kogels (%)', 'wp-maffia-game' ),
				'type'    => 'int',
				'default' => 25,
			),
		);
		...
		return array(
			'families' => array(
				...
				'columns'    => array(
					...
					'level'       => array( 'label' => __( 'Niveau', 'wp-maffia-game' ), 'type' => 'int' ),
					'money'       => array( 'label' => __( 'Kas', 'wp-maffia-game' ), 'type' => 'int' ),
					'bullets'     => array( 'label' => __( 'Kogels', 'wp-maffia-game' ), 'type' => 'int' ),
					'description' => array( 'label' => __( 'Profiel', 'wp-maffia-game' ), 'type' => 'textarea' ),
				),
			),
		);
	}
	...
	public function boot(): void {
		...
		add_filter(
			...
			function ( $fields, Character $c ) {
				...
				if ( $family ) {
					$fields[ __( 'Familie', 'wp-maffia-game' ) ] = '<a href="' . esc_url( $this->url( array( 'view' => 'family', 'id' => $family['id'] ) ) ) . '">' . esc_html( $family['name'] ) . '</a>';
				}
				return $fields;
			},
			10,
		...
		add_filter(
			...
			static function ( $allowed, Character $attacker, Character $target ) {
				...
				if ( true === $allowed && $mine && ( self::family_of( $target )['id'] ?? 0 ) === $mine['id'] ) {
					return new \WP_Error( 'family', __( 'Je vermoordt geen familie.', 'wp-maffia-game' ) );
				}
				return $allowed;
			},
			10,
	...
	public function on_killed( Character $victim, ?Character $killer ): void {
		...
		self::log( (int) $family['id'], 0, sprintf( __( '%s is vermoord.', 'wp-maffia-game' ), $victim->name ) );
		...
		if ( (int) $family['boss_id'] === $victim->id() ) {
			...
			if ( $heir && $heir->is_alive() && (int) $family['underboss_id'] !== $victim->id() ) {
				...
				$heir->notify( __( 'Je baas is vermoord. Jij leidt nu de familie!', 'wp-maffia-game' ) );
				/* translators: %s: player */
				self::log( (int) $family['id'], $heir->id(), sprintf( __( '%s is de nieuwe baas.', 'wp-maffia-game' ), $heir->name ) );
			} else {
				self::dissolve( (int) $family['id'], __( 'Je familie is opgeheven: de baas is vermoord zonder opvolger.', 'wp-maffia-game' ) );
			}
		}
	}
	...
	public static function permissions(): array {
		return apply_filters(
			...
			array(
				'invite'           => __( 'Leden uitnodigen', 'wp-maffia-game' ),
				'kick'             => __( 'Leden verwijderen', 'wp-maffia-game' ),
				'upgrade'          => __( 'Familie uitbreiden', 'wp-maffia-game' ),
				'withdraw_money'   => __( 'Geld opnemen uit de kas', 'wp-maffia-game' ),
				'withdraw_bullets' => __( 'Kogels opnemen uit de kas', 'wp-maffia-game' ),
				'edit_profile'     => __( 'Profiel bewerken', 'wp-maffia-game' ),
				'edit_internal'    => __( 'Interne mededeling bewerken', 'wp-maffia-game' ),
				'logs'             => __( 'Logboek bekijken', 'wp-maffia-game' ),
			)
		);
	}
	...
	public static function role( array $family, int $character_id ): string {
		if ( (int) $family['boss_id'] === $character_id ) {
			return __( 'Baas', 'wp-maffia-game' );
		}
		if ( (int) $family['underboss_id'] === $character_id ) {
			return __( 'Onderbaas', 'wp-maffia-game' );
		}
		return __( 'Lid', 'wp-maffia-game' );
	}
	...
	public function menu( Character $c ): array {
		$items   = array(
			array(
				'label' => __( 'Alle families', 'wp-maffia-game' ),
				'group' => 'family',
				'order' => 10,
			),
		);
		...
		if ( $family ) {
			$items[] = array(
				'label' => __( 'Mijn familie', 'wp-maffia-game' ),
				'group' => 'family',
				'order' => 20,
	...
	private function require_family( Character $c, string $permission = '' ): ?array {
		$family = self::family_of( $c );
		if ( ! $family ) {
			$this->error( __( 'Je zit niet in een familie.', 'wp-maffia-game' ) );
			return null;
		}
		if ( $permission && ! self::can( $c, $permission ) ) {
			$this->error( __( 'Daar heb je binnen de familie geen rechten voor.', 'wp-maffia-game' ) );
			return null;
		}
		return $family;
	}
	...
	public function action_create( Character $c, array $input ) {
		...
		$cost     = (int) $this->setting( 'family_cost' );
		if ( self::family_of( $c ) ) {
			$this->error( __( 'Je zit al in een familie.', 'wp-maffia-game' ) );
			return;
		}
		if ( mb_strlen( $name ) < 3 || mb_strlen( $name ) > 30 ) {
			$this->error( __( 'Een familienaam is 3 tot 30 tekens lang.', 'wp-maffia-game' ) );
			return;
		}
		if ( ! Locations::get( $location ) ) {
			$this->error( __( 'Kies een stad.', 'wp-maffia-game' ) );
			return;
		}
		if ( DB::value( 'SELECT id FROM {families} WHERE name = %s', $name ) ) {
			$this->error( __( 'Die naam is al bezet.', 'wp-maffia-game' ) );
			return;
		}
		if ( $this->setting( 'family_one_per_city' ) && DB::value( 'SELECT id FROM {families} WHERE location_id = %d', $location ) ) {
			$this->error( __( 'In deze stad heerst al een familie.', 'wp-maffia-game' ) );
			return;
		}
		if ( ! $c->spend( 'money', $cost ) ) {
			/* translators: %s: money */
			$this->error( sprintf( __( 'Een familie oprichten kost %s contant.', 'wp-maffia-game' ), Format::money( $cost ) ) );
			return;
		}
		$id = DB::insert(
			'families',
		...
		/* translators: %s: player */
		self::log( $id, $c->id(), sprintf( __( '%s richtte de familie op.', 'wp-maffia-game' ), $c->name ) );
		$c->log( 'family.create', true, $cost, $id );
		/* translators: %s: family */
		$this->success( sprintf( __( 'De familie %s is opgericht. Jij bent de baas.', 'wp-maffia-game' ), $name ) );
		return $this->home();
	}
	...
	public function action_invite( Character $c, array $input ): array {
		...
		if ( ! $target || ! $target->is_alive() ) {
			...
			$this->error( __( 'Deze speler zit al in een familie.', 'wp-maffia-game' ) );
		} elseif ( DB::value( 'SELECT id FROM {family_invites} WHERE family_id = %d AND character_id = %d', $family['id'], $target->id() ) ) {
			$this->error( __( 'Deze speler is al uitgenodigd.', 'wp-maffia-game' ) );
		} else {
			DB::insert(
				'family_invites',
				array(
			...
			/* translators: %s: family */
			$target->notify( sprintf( __( 'Je bent uitgenodigd voor de familie %s. Bekijk het onder Families.', 'wp-maffia-game' ), esc_html( $family['name'] ) ) );
			/* translators: 1: player, 2: player */
			self::log( (int) $family['id'], $c->id(), sprintf( __( '%1$s nodigde %2$s uit.', 'wp-maffia-game' ), $c->name, $target->name ) );
			$this->success( __( 'Uitnodiging verstuurd.', 'wp-maffia-game' ) );
		}
		return $this->home();
	}
	...
	public function action_accept( Character $c, array $input ) {
		...
		if ( ! $family ) {
			$this->error( __( 'Deze uitnodiging is niet meer geldig.', 'wp-maffia-game' ) );
			return;
		}
		if ( self::family_of( $c ) ) {
			$this->error( __( 'Je zit al in een familie.', 'wp-maffia-game' ) );
		...
		if ( count( self::members( (int) $family['id'] ) ) >= $this->capacity( $family ) ) {
			$this->error( __( 'Deze familie zit vol.', 'wp-maffia-game' ) );
			return;
		}
		DB::insert(
			'family_members',
		...
		DB::delete( 'family_invites', array( 'character_id' => $c->id() ) );
		/* translators: %s: player */
		self::log( (int) $family['id'], $c->id(), sprintf( __( '%s is lid geworden.', 'wp-maffia-game' ), $c->name ) );
		/* translators: %s: family */
		$this->success( sprintf( __( 'Welkom bij %s.', 'wp-maffia-game' ), $family['name'] ) );
		return $this->home();
	}
	...
	public function action_leave( Character $c, array $input ) {
		...
		if ( (int) $family['boss_id'] === $c->id() ) {
			$this->error( __( 'Een baas vertrekt niet. Draag de leiding over of hef de familie op.', 'wp-maffia-game' ) );
			return $this->home();
		}
		...
		/* translators: %s: player */
		self::log( (int) $family['id'], $c->id(), sprintf( __( '%s heeft de familie verlaten.', 'wp-maffia-game' ), $c->name ) );
		$this->success( __( 'Je hebt de familie verlaten.', 'wp-maffia-game' ) );
	}
	...
	public function action_kick( Character $c, array $input ): array {
		...
		if ( in_array( $target->id(), array( (int) $family['boss_id'], $c->id() ), true ) ) {
			$this->error( __( 'Dat lid kun je niet verwijderen.', 'wp-maffia-game' ) );
			return $this->home();
		}
		if ( DB::delete( 'family_members', array( 'character_id' => $target->id(), 'family_id' => $family['id'] ) ) ) {
			...
			/* translators: %s: family */
			$target->notify( sprintf( __( 'Je bent uit de familie %s gezet.', 'wp-maffia-game' ), esc_html( $family['name'] ) ) );
			/* translators: 1: player, 2: player */
			self::log( (int) $family['id'], $c->id(), sprintf( __( '%1$s zette %2$s uit de familie.', 'wp-maffia-game' ), $c->name, $target->name ) );
		}
		return $this->home();
	}
	...
	public function action_roles( Character $c, array $input ): array {
		...
		if ( (int) $family['boss_id'] !== $c->id() ) {
			$this->error( __( 'Alleen de baas verdeelt de rollen.', 'wp-maffia-game' ) );
			return $this->home();
		}
		...
		if ( 'underboss' === $role && $member->id() !== $c->id() ) {
			...
			/* translators: %s: player */
			self::log( (int) $family['id'], $c->id(), sprintf( __( '%s is benoemd tot onderbaas.', 'wp-maffia-game' ), $member->name ) );
			...
			/* translators: %s: player */
			self::log( (int) $family['id'], $c->id(), sprintf( __( '%s is de nieuwe baas.', 'wp-maffia-game' ), $member->name ) );
			$member->notify( __( 'Je bent de nieuwe baas van je familie.', 'wp-maffia-game' ) );
		...
		$this->success( __( 'Rechten bijgewerkt.', 'wp-maffia-game' ) );
		return $this->home();
	}
	...
	public function action_deposit( Character $c, array $input ): array {
		...
		if ( $amount < 1 || ! $c->spend( $field, $amount ) ) {
			$this->error( __( 'Zoveel heb je niet.', 'wp-maffia-game' ) );
			return $this->home();
		}
		...
		$label = 'money' === $field ? Format::money( $credit ) : sprintf( /* translators: %s: number */ __( '%s kogels', 'wp-maffia-game' ), Format::number( $credit ) );
		/* translators: 1: player, 2: amount */
		self::log( (int) $family['id'], $c->id(), sprintf( __( '%1$s stortte %2$s in de kas.', 'wp-maffia-game' ), $c->name, $label ) );
		/* translators: %s: amount */
		$this->success( sprintf( __( 'Er is %s in de familiekas gestort.', 'wp-maffia-game' ), $label ) );
		return $this->home();
	}
	...
	public function action_withdraw( Character $c, array $input ): array {
		...
		if ( ! $done ) {
			$this->error( __( 'Zoveel zit er niet in de kas.', 'wp-maffia-game' ) );
			return $this->home();
		}
		$c->add( $field, $amount );
		$label = 'money' === $field ? Format::money( $amount ) : sprintf( /* translators: %s: number */ __( '%s kogels', 'wp-maffia-game' ), Format::number( $amount ) );
		/* translators: 1: player, 2: amount */
		self::log( (int) $family['id'], $c->id(), sprintf( __( '%1$s nam %2$s op uit de kas.', 'wp-maffia-game' ), $c->name, $label ) );
		/* translators: %s: amount */
		$this->success( sprintf( __( 'Je nam %s op.', 'wp-maffia-game' ), $label ) );
		return $this->home();
	}
	...
	public function action_upgrade( Character $c, array $input ): array {
		...
		if ( ! $done ) {
			/* translators: %s: money */
			$this->error( sprintf( __( 'Uitbreiden kost %s uit de familiekas.', 'wp-maffia-game' ), Format::money( $cost ) ) );
			return $this->home();
		}
		/* translators: 1: player, 2: money */
		self::log( (int) $family['id'], $c->id(), sprintf( __( '%1$s breidde de familie uit voor %2$s.', 'wp-maffia-game' ), $c->name, Format::money( $cost ) ) );
		$this->success( __( 'De familie heeft een extra plek.', 'wp-maffia-game' ) );
		return $this->home();
	}
	...
	public function action_disband( Character $c, array $input ) {
		...
		if ( ! $family || (int) $family['boss_id'] !== $c->id() ) {
			$this->error( __( 'Alleen de baas kan de familie opheffen.', 'wp-maffia-game' ) );
			return;
		}
		if ( empty( $input['confirm'] ) ) {
			$this->error( __( 'Vink de bevestiging aan.', 'wp-maffia-game' ) );
			return $this->home();
		}
		$c->add( 'money', (int) $family['money'] );
		$c->add( 'bullets', (int) $family['bullets'] );
		/* translators: %s: family */
		self::dissolve( (int) $family['id'], sprintf( __( 'De familie %s is opgeheven door de baas.', 'wp-maffia-game' ), esc_html( $family['name'] ) ) );
		$this->success( __( 'De familie is opgeheven. De kas is naar jou gegaan.', 'wp-maffia-game' ) );
	}
}
