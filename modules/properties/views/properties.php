...
?>
<h3><?php esc_html_e( 'Your properties', 'wp-maffia-game' ); ?></h3>
<?php if ( ! $mine ) : ?>
	<?php echo UI::empty_state( __( 'You don\'t own anything yet. Businesses without an owner can be bought on the spot.', 'wp-maffia-game' ) ); // phpcs:ignore ?>
<?php endif; ?>
<div class="dfmg-grid dfmg-grid--2">
	<?php foreach ( $mine as $dfmg_p ) : ?>
		...
		<section class="dfmg-card">
			...
			<p><?php esc_html_e( 'Profit since takeover:', 'wp-maffia-game' ); ?> <strong><?php echo esc_html( Format::money( $dfmg_p->profit() ) ); ?></strong></p>
			...
			<?php echo $this->form( 'transfer', $dfmg_hidden ); // phpcs:ignore ?>
				<input type="text" name="to" placeholder="<?php esc_attr_e( 'Transfer to (name)', 'wp-maffia-game' ); ?>" required>
				<button type="submit" class="dfmg-button dfmg-button--ghost"><?php esc_html_e( 'Transfer', 'wp-maffia-game' ); ?></button>
			</form>
			<div class="dfmg-actions">
				<?php echo $this->button( 'reset', __( 'Reset profit', 'wp-maffia-game' ), $dfmg_hidden, 'dfmg-button dfmg-button--ghost dfmg-button--small' ); // phpcs:ignore ?>
				<?php echo $this->button( 'drop', __( 'Give up', 'wp-maffia-game' ), $dfmg_hidden, 'dfmg-button dfmg-button--danger dfmg-button--small' ); // phpcs:ignore ?>
			</div>
		</section>
	<?php endforeach; ?>
</div>

<h3><?php esc_html_e( 'Owners around the world', 'wp-maffia-game' ); ?></h3>
<table class="dfmg-table">
	<thead>
		<tr>
			<th><?php esc_html_e( 'Business', 'wp-maffia-game' ); ?></th>
			<th><?php esc_html_e( 'City', 'wp-maffia-game' ); ?></th>
			<th><?php esc_html_e( 'Owner', 'wp-maffia-game' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php foreach ( Locations::all() as $dfmg_loc ) : ?>
			<?php foreach ( $types as $dfmg_key => $dfmg_type ) : ?>
				...
				<tr>
					...
					<td><?php echo ( $dfmg_owner_c && $dfmg_owner_c->is_alive() ) ? $dfmg_owner_c->link() : '<em class="dfmg-muted">' . esc_html__( 'for sale', 'wp-maffia-game' ) . '</em>'; // phpcs:ignore ?></td>
				</tr>
			<?php endforeach; ?>
		<?php endforeach; ?>
	</tbody>
