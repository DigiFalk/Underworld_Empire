...
<?php if ( ! $topics ) : ?>
	<?php echo UI::empty_state( __( 'Nog geen topics.', 'wp-maffia-game' ) ); // phpcs:ignore ?>
<?php else : ?>
	<table class="dfmg-table dfmg-forum">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Topic', 'wp-maffia-game' ); ?></th>
				<th><?php esc_html_e( 'Door', 'wp-maffia-game' ); ?></th>
				<th><?php esc_html_e( 'Posts', 'wp-maffia-game' ); ?></th>
				<th><?php esc_html_e( 'Laatste post', 'wp-maffia-game' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php foreach ( $topics as $dfmg_t ) : ?>
				<tr>
					<td>
						<?php if ( $dfmg_t['sticky'] ) : ?><em class="dfmg-tag"><?php esc_html_e( 'vast', 'wp-maffia-game' ); ?></em><?php endif; ?>
						<?php if ( $dfmg_t['locked'] ) : ?><em class="dfmg-tag"><?php esc_html_e( 'gesloten', 'wp-maffia-game' ); ?></em><?php endif; ?>
...
<?php if ( $can_post ) : ?>
	<div class="dfmg-card">
		<h3><?php esc_html_e( 'Nieuw topic', 'wp-maffia-game' ); ?></h3>
		<?php echo $this->form( 'new_topic', array( 'board' => $board['id'] ), 'dfmg-form dfmg-form--stacked' ); // phpcs:ignore ?>
			<input type="text" name="title" maxlength="150" placeholder="<?php esc_attr_e( 'Titel', 'wp-maffia-game' ); ?>" required>
