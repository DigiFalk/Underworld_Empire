...
| --- | --- |
| `wp-content/plugins/wp-maffia-game/modules/<id>/` | Bundled modules. Don't edit: changes are lost on update. |
| `wp-content/maffia-modules/<id>/` | **Your own modules.** Survives updates. A module with the same id as a bundled module replaces it. Change the folder with `define( 'DFMG_CUSTOM_MODULES_DIR', '/path' );` in `wp-config.php`. |
...
`wp-content/maffia-modules/hello/module.php`:
...
The file must return an object that extends `DigiFalk\MaffiaGame\Module\Module`.
...
| Code | What |
| --- | --- |
| `$this->view( 'file', $vars )` | Renders `views/file.php`. Inside the template `$this` is the module. Themes can override it in `<theme>/wp-maffia-game/<id>/file.php`. |
...
| `UI::cooldown()`, `UI::bar()`, `UI::pager()`, `UI::property()` | Ready-made components (`DigiFalk\MaffiaGame\Frontend\UI`). |
...
### The character (`DigiFalk\MaffiaGame\Character`)
