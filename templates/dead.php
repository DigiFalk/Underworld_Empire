...
<div class="dfmg-welcome">
	<div class="dfmg-card dfmg-card--dead">
		...
		<h2><?php esc_html_e( 'Rest in peace', 'wp-maffia-game' ); ?></h2>
		<p>
			<?php
			if ( $killer ) {
				/* translators: 1: your character, 2: killer */
				printf( esc_html__( '%1$s was murdered by %2$s.', 'wp-maffia-game' ), '<strong>' . esc_html( $character->name ) . '</strong>', $killer->link() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			} else {
				/* translators: %s: your character */
				printf( esc_html__( '%s is no longer with us.', 'wp-maffia-game' ), '<strong>' . esc_html( $character->name ) . '</strong>' );
			}
			?>
		</p>
		<p><?php esc_html_e( 'Your premium points are kept. Start again with a new character.', 'wp-maffia-game' ); ?></p>
		<?php echo Game::form_open( 'core', 'create_character' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<label for="dfmg-name"><?php esc_html_e( 'New name', 'wp-maffia-game' ); ?></label>
			<input type="text" id="dfmg-name" name="name" minlength="3" maxlength="20" pattern="[A-Za-z0-9_\-]{3,20}" required>
			<button type="submit" class="dfmg-button"><?php esc_html_e( 'Start over', 'wp-maffia-game' ); ?></button>
		</form>
	</div>
</div>
