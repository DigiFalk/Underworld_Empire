 ...
 * Game layout.
 *
 * @var \DigiFalk\MafiaGame\Character     $character
 * @var \DigiFalk\MafiaGame\Module\Module $module
 * @var array                              $menu
 * @var array                              $messages
 * @var string                             $content
 *
...
$dfmg_stats = apply_filters(
	'dfmg_header_stats',
	array(
		'rank'     => array( __( 'Rank', 'wp-mafia-game' ), esc_html( $character->rank_name() ) ),
		'money'    => array( __( 'Cash', 'wp-mafia-game' ), esc_html( Format::money( $character->money ) ) ),
		'bank'     => array( __( 'Bank', 'wp-mafia-game' ), esc_html( Format::money( $character->bank ) ) ),
		'bullets'  => array( __( 'Bullets', 'wp-mafia-game' ), esc_html( Format::number( $character->bullets ) ) ),
		'health'   => array( __( 'Health', 'wp-mafia-game' ), esc_html( $character->health_percent() . '%' ) ),
		'location' => array( __( 'City', 'wp-mafia-game' ), esc_html( $character->location_name() ) ),
		'points'   => array( esc_html( (string) \DigiFalk\MafiaGame\Settings::get( 'points_name' ) ), esc_html( Format::number( $character->points ) ) ),
	),
	$character
);
?>
<div class="dfmg-shell">
	<header class="dfmg-header">
		<div class="dfmg-header__who">
			...
			<div class="dfmg-progress" title="<?php echo esc_attr( sprintf( /* translators: %s: percent */ __( '%s%% to next rank', 'wp-mafia-game' ), $character->rank_progress() ) ); ?>">
		...
		<button type="button" class="dfmg-menu-toggle" aria-controls="dfmg-nav" aria-expanded="false"><?php esc_html_e( 'Menu', 'wp-mafia-game' ); ?></button>
	</header>
	...
	<div class="dfmg-body">
		<nav class="dfmg-nav" id="dfmg-nav">
			...
			<div class="dfmg-nav__group">
				<ul>
					<li><a href="<?php echo esc_url( wp_logout_url( Game::page_url() ) ); ?>"><?php esc_html_e( 'Log out', 'wp-mafia-game' ); ?></a></li>
				</ul>
			</div>
		</nav>
	...
	<footer class="dfmg-footer">
		<?php echo esc_html( (string) \DigiFalk\MafiaGame\Settings::get( 'round_name' ) ); ?> &middot; WP Mafia Game &copy; DigiFalk
	</footer>
</div>
